...
# Test different types of check marks
@pytest.mark.asyncio
@pytest.mark.parametrize("check_mark", [
    ...
    "🗸",   # Light Check Mark
    "\u2705", # Unicode for White Heavy Check Mark
    "\u2714\uFE0F", # Unicode for Heavy Check Mark with variation selector
    "\u2714", # Unicode for Heavy Check Mark
    "\u2713", # Unicode for Check Mark
    "\u2611\uFE0F", # Unicode for Ballot Box with Check with variation selector
    "\u2611", # Unicode for Ballot Box with Check
    "\u1F5F8" # Unicode for Light Check Mark
])
async def test_different_check_marks(reset_data, mock_update, mock_context, check_mark):
    # Set up checkmark message
    mock_update.message.text = f"Done {check_mark}"
    
    # Reset reply mock for each call
